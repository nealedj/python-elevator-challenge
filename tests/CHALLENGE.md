...
*This document is the original challenge this repository grew from, and it doubles as the first test suite. The project overview lives in the [top-level README](../README.md).*
...
A naive implementation of the business logic is provided in the `elevators/elevator.py` file in this project. If you run `doctest` using the provided implementation, several examples fail to produce the expected output. Your challenge is to fix that implementation until all of the examples pass.

Open a pull request with your solution. Good luck! Have fun!

Once the tests here pass, there is a second, more realistic suite in [SCENARIOS.md](SCENARIOS.md): it simulates passengers arriving over time — rush hours, near misses, and randomized stress tests — and asserts that everybody is delivered within bounded time. It also covers an alternative dispatcher in `elevators/efficient_elevator.py` that cuts waiting times on that passenger model; `python -m elevators.benchmark` compares the two head to head. [CLUSTER.md](CLUSTER.md) goes further still: a bank of six elevators with a group dispatcher serving an eleven-story office tower. And [DESTINATION.md](DESTINATION.md) is the endgame: a destination dispatch system putting that bank through a full working day of a 5,000-person office building.
...
As for the business logic, an example implementation is provided in the `elevators/elevator.py` file in this project.

    >>> from elevators import ElevatorLogic

As provided, it doesn't pass the tests in this document. Your challenge is to fix it so that it does. To run the tests, run this from the repository root:

    python -m doctest tests/CHALLENGE.md -o NORMALIZE_WHITESPACE

Add `-v` for a verbose report of every example that runs.
